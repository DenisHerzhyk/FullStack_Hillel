...
function setUserInfoCookie(key, value) {
  ...
  const expireDate = new Date(Date.now() + 10000).toUTCString()
  document.cookie = `userInfo=${encodeURIComponent(userInfo)}`
...
function getUserInfo(key) {
  let gettingData = sessionStorage.getItem(key)
  console.log(`Retrieved ${key}: ${gettingData}`)
  return gettingData
}
